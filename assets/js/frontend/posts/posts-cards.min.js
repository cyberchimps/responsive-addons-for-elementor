class RaelCardsHandler extends elementorModules.frontend.handlers.Base{getSkinPrefix(){return'rael_cards_'}
...
applySimpleMasonry(container,colsCount,gap){if(!container)return;var items=container.querySelectorAll('.elementor-post');if(items.length===0)return;var containerWidth=container.offsetWidth;if(containerWidth===0){var self=this;setTimeout(function(){self.applySimpleMasonry(container,colsCount,gap)},200);return}
var colWidth=(containerWidth-(gap*(colsCount-1)))/colsCount;container.style.position='relative';container.style.height='auto';items.forEach(function(item){item.style.position='relative';item.style.width=colWidth+'px';item.style.float='left';item.style.marginRight=gap+'px';item.style.marginBottom=gap+'px';item.style.boxSizing='border-box'});container.offsetHeight;var colHeights=new Array(colsCount).fill(0);items.forEach(function(item){item.style.position='absolute';item.style.float='';item.style.marginRight='';item.style.marginBottom='';item.style.transition='transform 0.3s ease';var shortestCol=0;for(var i=1;i<colsCount;i++){if(colHeights[i]<colHeights[shortestCol]){shortestCol=i}}
var left=shortestCol*(colWidth+gap);var top=colHeights[shortestCol];item.style.left=left+'px';item.style.top=top+gap+'px';colHeights[shortestCol]+=item.offsetHeight+gap});var maxHeight=Math.max(...colHeights);container.style.height=maxHeight+'px';var self=this;setTimeout(function(){var newContainerWidth=container.offsetWidth;if(newContainerWidth>0&&newContainerWidth!==containerWidth){self.applySimpleMasonry(container,colsCount,gap)}},300)}
...
function refreshRaelMasonry($widget){var widgetId=$widget.data('id');var handlers=elementorFrontend.elementsHandler.handlers;for(var key in handlers){if(handlers[key]&&handlers[key].$element&&handlers[key].$element[0]===$widget[0]){if(handlers[key].runMasonry){setTimeout(function(){handlers[key].runMasonry()},800)}
break}}}
...
callAjax(term,postPerPage,paged,pid,$scope,skin)});$('body').on('click','.rael-post-pagination .page-numbers',function(e){let $scope=$(this).closest('.elementor-widget-rael-posts');if($scope.length>0){e.preventDefault()}
$('.rael-post-pagination span.elementor-screen-only').remove();var page_number=1;var curr=parseInt($scope.find('.rael-post-pagination .page-numbers.current').html());var $this=$(this);if($this.hasClass('next')){page_number=curr+1}else if($this.hasClass('prev')){page_number=curr-1}else{page_number=$this.html()}
if($scope.find('.responsive-posts-container').data('pagination')==='prev_next'){page_number=$scope.find('.responsive-posts-container').data('paged');if($this.hasClass('next')){page_number+=1}else{page_number-=1}
...
var postPerPage=$scope.find('.responsive-posts-container').data('post-per-page');paged_no+=1;var paged=paged_no;$scope.find('.responsive-posts-container').data('paged',paged);let widget_id=$scope.data('id');$.ajax({type:'POST',url:localize.ajaxurl,data:{action:'rael_get_posts_by_terms',data:{term,postPerPage,paged,pid,widget_id,skin}},success:function success(data){var sel=$scope.find('.responsive-posts-container');if(sel.data('pagination')==='infinite'){$scope.find('.responsive-post-load-more-loader').remove()
...
sel.next('.rael-post-pagination').first().remove();$(data.pagination).insertAfter(sel);$('div.responsive-post-loader').remove()}})}
window.addEventListener('load',function(){setTimeout(function(){$('.elementor-widget-rael-posts').each(function(){var $widget=$(this);setTimeout(function(){refreshRaelMasonry($widget)},800)})},1500)});window.addEventListener('resize',function(){setTimeout(function(){$('.elementor-widget-rael-posts').each(function(){var $widget=$(this);setTimeout(function(){refreshRaelMasonry($widget)},200)})},200)})

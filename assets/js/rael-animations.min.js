(function($){'use strict';function getRealContainer($scope){if($scope.hasClass('e-con'))return $scope;if($scope.is('section.elementor-section'))return $scope;const $column=$scope.closest('.elementor-column');if($column.length)return $column;return $scope}
...
$(window).on('elementor/frontend/init',()=>{elementorFrontend.hooks.addAction('frontend/element_ready/global',($scope)=>{this.processElement($scope)});if(this.isEditor){elementor.channels.editor.on('change',(model)=>{setTimeout(()=>{this.scanElements();this.update();this.updateEntranceAnimations()},50)})}});this.rafUpdate=()=>{this.update();this.rafId=requestAnimationFrame(this.rafUpdate)};this.rafId=requestAnimationFrame(this.rafUpdate)},onScroll(){this.scrollTop=this.getScrollTop()},onResize(){this.windowHeight=this.getViewportHeight();this.scanElements()},scanElements(){this.elements=[];document.querySelectorAll('.rael-scroll-effects').forEach(element=>{this.elements.push(element)})},processElement($scope){const $target=getRealContainer($scope);const settings=getElementSettings($scope);const effects=buildEffectsFromSettings(settings);if(effects){$target.addClass('rael-scroll-effects');$target.attr('data-rael-scroll-effects',JSON.stringify({effects:effects,relativeTo:settings.rae_animations_effects_relative_to||'viewport'}));if(!this.elements.includes($target[0])){this.elements.push($target[0])}}
this.setupEntranceAnimation($target,settings)},setupEntranceAnimation($element,settings){if(settings.rae_animations_entrance&&settings.rae_animations_entrance!=='none'){$element.removeClass('animated rael-animated');$element.removeClass(settings.rae_animations_entrance);$element.addClass('rael-entrance');$element.attr('data-rae-entrance',settings.rae_animations_entrance);const duration=settings.rae_animations_entrance_duration||'1000';$element.attr('data-rae-animation-duration',duration);$element.removeClass('rae-duration-slow rae-duration-normal rae-duration-fast');if(duration==='2000'){$element.addClass('rae-duration-slow')}else if(duration==='1000'){$element.addClass('rae-duration-normal')}else if(duration==='800'){$element.addClass('rae-duration-fast')}
const delay=settings.rae_animations_entrance_animation_delay||'0';$element.attr('data-rae-animation-delay',delay);$element.removeClass('animated');$element[0].__raeEntranceDone=!1}else{$element.removeClass('rael-entrance animated rael-animated');$element.removeAttr('data-rae-entrance data-rae-animation-duration data-rae-animation-delay')}},update(){if(!this.elements.length)return;this.elements.forEach(element=>{this.applyEffects(element)});this.handleEntranceAnimations()},applyEffects(element){let effectsData;if(this.isEditor){const $scope=$(element);const settings=getElementSettings($scope);const effects=buildEffectsFromSettings(settings);if(!effects){$scope.removeClass('rael-scroll-effects');return}
...
break}
progress=Math.min(Math.max(progress,0),1);return progress},applyEffect(element,effectType,effectConfig,progress){if(!effectConfig)return;const mappedProgress=this.range(progress,effectConfig.start,effectConfig.end);switch(effectType){case 'translateX':this.applyTranslateX(element,effectConfig,mappedProgress);break;case 'translateY':this.applyTranslateY(element,effectConfig,mappedProgress);break;case 'opacity':this.applyOpacity(element,effectConfig,mappedProgress);break;case 'blur':this.applyBlur(element,effectConfig,mappedProgress);break;case 'scale':this.applyScale(element,effectConfig,mappedProgress);break;case 'rotate':this.applyRotate(element,effectConfig,mappedProgress);break}},applyTranslateX(element,config,progress){if(!config||typeof config.speed==='undefined')return;let value=(1-progress)*config.speed*20;if(config.direction==='to_left'){value=-Math.abs(value)}else if(config.direction==='to_right'){value=Math.abs(value)}
element.style.setProperty('--translateX',`${value}px`)},applyTranslateY(element,config,progress){if(!config||typeof config.speed==='undefined')return;let value=(1-progress)*config.speed*20;if(config.direction==='up'){value=Math.abs(value)}else if(config.direction==='down'){value=-Math.abs(value)}
...
element.style.setProperty('--rotateZ',`${rotate}deg`)},range(progress,start,end){const s=start/100;const e=end/100;if(progress<=s)return 0;if(progress>=e)return 1;return(progress-s)/(e-s)},handleEntranceAnimations(){const self=this;document.querySelectorAll('.rael-entrance:not(.animated)').forEach(element=>{const animation=element.getAttribute('data-rae-entrance');if(!animation||animation==='none')return;const rect=element.getBoundingClientRect();const isInViewport=(rect.top<=(window.innerHeight||document.documentElement.clientHeight)&&rect.bottom>=0&&rect.left<=(window.innerWidth||document.documentElement.clientWidth)&&rect.right>=0);if(isInViewport&&!element.__raeEntranceDone){this.triggerEntranceAnimation(element)}})},triggerEntranceAnimation(element){const animation=element.getAttribute('data-rae-entrance');const delay=element.getAttribute('data-rae-animation-delay')||0;const duration=element.getAttribute('data-rae-animation-duration')||'1000';setTimeout(()=>{element.classList.add('animated',animation,'rael-animated');element.style.animationDuration=duration+'ms';element.__raeEntranceDone=!0;const animationDuration=parseInt(duration)+parseInt(delay);setTimeout(()=>{element.classList.remove(animation)},animationDuration)},parseInt(delay))},updateEntranceAnimations(){document.querySelectorAll('.rael-entrance').forEach(element=>{element.classList.remove('animated','rael-animated');const currentAnimation=element.getAttribute('data-rae-entrance');if(currentAnimation){element.classList.remove(currentAnimation)}
element.__raeEntranceDone=!1})},destroy(){if(this.rafId){cancelAnimationFrame(this.rafId)}
$(window).off('scroll resize')}};$(window).on('elementor/frontend/init',function(){RaelAnimations.init();elementorFrontend.hooks.addAction('frontend/element_ready/global',function($scope){RaelAnimations.processElement($scope);RaelAnimations.scanElements()});elementorFrontend.hooks.addAction('frontend/element_ready/section',function($scope){RaelAnimations.processElement($scope);RaelAnimations.scanElements()});elementorFrontend.hooks.addAction('frontend/element_ready/container',function($scope){RaelAnimations.processElement($scope);RaelAnimations.scanElements()});elementorFrontend.hooks.addAction('frontend/element_ready/column',function($scope){RaelAnimations.processElement($scope);RaelAnimations.scanElements()})});$(document).ready(function(){if(typeof elementorFrontend!=='undefined'){$(window).trigger('elementor/frontend/init')}})})(jQuery)